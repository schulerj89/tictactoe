...
test("SoundManager resolves music assets relative to the configured base URL", () => {
  ...
  assert.equal(track.source, "/tictactoe/audio/ode-to-joy-32bit.wav");
});
...
test("SoundManager falls back to a root-relative base URL", () => {
  ...
  assert.equal(track.source, "/audio/fur-elise-32bit.wav");
});
...
test("SoundManager prefers document.baseURI when deriving the deployed base path", () => {
  ...
  try {
    ...
    assert.equal(track.source, "/tictactoe/audio/minuet-in-g-32bit.wav");
  } finally {
    globalThis.window = originalWindow;
    globalThis.document = originalDocument;
  }
...
test("SoundManager applies configured volume levels", () => {
  ...
  try {
    ...
    const audio = manager.ensureMusicAudio("/audio/fur-elise-32bit.wav");
...
test("SoundManager exposes all bundled music tracks", () => {
  ...
  assert.equal(manager.getTrack("turkish-march").source, "/audio/turkish-march-32bit.wav");
});
